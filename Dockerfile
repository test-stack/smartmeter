...
ENV SMARTMETER_URL http://smartmeter-api.etnetera.cz/download/release/1.3.0/linux/full/SmartMeter_1.3.0_linux.tar.gz
ENV SMARTMETER_PATH /home/${SMARTMETER_DIRECTORY}_linux/

RUN apk update && apk upgrade && apk add tar curl ca-certificates && \
    ...
    apk del tar, curl ca-certificates
